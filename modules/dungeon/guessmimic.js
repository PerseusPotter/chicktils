...
const notifs = [false, false, false];
...
const scoreUpdateReg = reg('packetReceived', pack => {
  ...
  Client.scheduleTask(2, () => {
    ...
    if (diff > 0 && ScoreCalculation.getDeaths().get() > 0 && !ScoreCalculation.getFirstDeathHadSpirit().get()) {
      if (!notifs[0]) log(`&dGuessing that first death DID have spirit (score difference of ${diff})`);
      notifs[0] = true;
      ScoreCalculation.getFirstDeathHadSpirit().set(true);
      diff--;
    }
    if (diff >= 2 && !ScoreCalculation.getMimicKilled().get()) {
      if (!notifs[1]) log(`&dGuessing that mimic has been killed (score difference of ${diff})`);
      notifs[1] = true;
      ScoreCalculation.getMimicKilled().set(true);
      diff -= 2;
    }
    if (diff >= 1 && !statePrinceKilled.get()) {
      if (!notifs[2]) log(`&dGuessing that a prince has been killed (score difference of ${diff})`);
      notifs[2] = true;
