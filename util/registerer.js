...
import { log } from './log';
import { getBlockPos, stateSinglePlayer } from './mc';
...
const createRegister = function(type, shit) {
  if (PROFILER) {
    ...
    const orig = shit;
    const isRender = ['render', 'draw'].some(v => typeName.toLowerCase().includes(v));
    shit = function(...args) {
      start();
      orig.apply(this, args[0] === undefined && args.length === 1 ? [] : args);
      isRender ? endRender(id) : endTick(id);
    };
  }
...

{
  ...
  function ChickTilsBlockChange(cb) {
    ChickTilsRegister.call(this, cb);
  }
  inherits(ChickTilsBlockChange, ChickTilsRegister);
  ChickTilsBlockChange.list = new Map();
  listenList(ChickTilsBlockChange.list);
  ChickTilsBlockChange.prototype.getList = function getList() {
    return ChickTilsBlockChange.list;
  };
  ChickTilsBlockChange.blockChangeReg = reg('packetReceived', pack => {
    if (pack.func_179827_b) {
      const bp = pack.func_179827_b();
      const bs = pack.func_180728_a();
      const pos = getBlockPos(bp);
      ChickTilsBlockChange.list.forEach(v => v.cb(pos, bs, bp));
  ...
  }).setFilteredClasses([net.minecraft.network.play.server.S22PacketMultiBlockChange, net.minecraft.network.play.server.S23PacketBlockChange])
  ChickTilsBlockChange.prototype.update = function update() {
    if (ChickTilsBlockChange.list.size) {
      ChickTilsBlockChange.blockChangeReg.register();
    } else {
      ChickTilsBlockChange.blockChangeReg.unregister();
    }
  };
